...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Резервный пользователь (предотвращает зависания при падении сервера)
    let currentUser = {
        balance: 0.000,
    ...
    // Сброс сессии кошелька при смене аккаунтов Telegram
    try {
    ...
    // ==========================================================================
    // СТАБИЛЬНЫЙ TON CONNECT (ПОДКЛЮЧЕНИЕ КОШЕЛЬКА)
    // ==========================================================================
    ...
    try {
        ...
        if (typeof TON_CONNECT_UI !== 'undefined' && TON_CONNECT_UI.TonConnectUI) {
            tonConnectUI = new TON_CONNECT_UI.TonConnectUI({ manifestUrl: manifestUrl });
        } else if (window.TonConnectUI) {
            tonConnectUI = new window.TonConnectUI({ manifestUrl: manifestUrl });
        }
            ...
            if (elements.connectWalletBtn) {
                elements.connectWalletBtn.addEventListener('click', async () => {
                    try {
                        ...
                        showNotification("Не удалось запустить кошелёк", "⚠️");
                    }
                });
            }
    ...
    async function fetchUserData() {
        try {
            ...
            console.error("Сервер не ответил, использованы локальные данные Telegram:", e);
        }
