...
// ----------------- СОСТОЯНИЕ МУЛЬТИПЛЕЕРА ИГРЫ АРЕНЫ -----------------
let arenaState = {
    roundId: 175, 
    state: 'waiting', // waiting, countdown, running, ended
    countdownLeft: 15,
    bets: [], // { userId, username, firstName, avatarUrl, amount, color }
    ballAngle: 0,
    timerInterval: null
};
...
function startArenaCountdown() {
    ...
    arenaState.state = 'countdown';
    arenaState.countdownLeft = 15;

    arenaState.timerInterval = setInterval(async () => {
        arenaState.countdownLeft--;

        if (arenaState.countdownLeft <= 0) {
            clearInterval(arenaState.timerInterval);
            arenaState.timerInterval = null;
            await runArenaPhysicsAndReward();
        }
    }, 1000);
}
...
async function runArenaPhysicsAndReward() {
    ...
    try {
        ...
        await client.query(
            ...
            [winner.userId, 'arena_win', finalProfit, `Победа в Арене #${arenaState.roundId}. Шанс: ${((winnerBetAmount/totalBets)*100).toFixed(1)}%`]
        );
...
app.get('/api/arena/state', (req, res) => {
    res.json({
        ...
        countdownLeft: arenaState.countdownLeft,
        bets: arenaState.bets,
        ballAngle: arenaState.ballAngle
    });
});
...
app.post('/api/arena/bet', async (req, res) => {
    ...
    try {
        ...
        if (existingBet) {
            ...
            const nextColor = SECTOR_COLORS[arenaState.bets.length % SECTOR_COLORS.length];
            arenaState.bets.push({
                userId: userId,
                username: user.username || user.first_name || 'Игрок',
                firstName: user.first_name || 'Игрок',
                avatarUrl: user.avatar_url || "https://img.icons8.com/color/96/user.png",
                amount: amount,
                color: nextColor
            });
        }
