...
app.use(async (req, res, next) => {
    ...
    try {
        ...
        if (calculatedHash !== hash) {
            console.warn('Telegram InitData validation failed: Invalid hash.');
            req.telegramUser = null;
        ...
        console.error('Error validating Telegram InitData:', e.message);
        req.telegramUser = null;
...
// 2. Получение информации о ежедневном кейсе
...
app.post('/api/open_daily_case', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (drops.length === 0) {
                await client.query('ROLLBACK');
                return res.status(500).json({ error: 'Призы для кейса не настроены.' });
            }
