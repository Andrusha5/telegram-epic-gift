...
// Новый эндпоинт для динамической отдачи пула призов фронтенду
app.get('/api/gifts_pool', async (req, res) => {
    try {
        const itemsRows = (await query('SELECT id, name, image_url as icon, value as "rawPrice", type FROM items')).rows;
        // Мапим данные под формат фронтенда
        const poolData = itemsRows.map(item => ({
            id: item.id,
            name: item.name,
            icon: item.icon,
            price: `${item.rawPrice} TON`,
            rawPrice: parseFloat(item.rawPrice),
            isGold: parseFloat(item.rawPrice) >= 5.0, // Золотой тир динамически от 5 TON
            type: item.type
        }));
        res.json(poolData);
    } catch (error) {
        res.status(500).json({ error: 'Ошибка сервера при получении призов.' });
    }
});
...
app.post('/api/open_daily_case', async (req, res) => {
    ...
    if (!isSubscribed) {
        return res.status(403).json({ error: "Для открытия... @" + CHANNEL_USERNAME });
    }
    ...
    try {
        ...
        if (!user.is_admin && timeElapsed < cooldown) {
            await client.query('ROLLBACK');
            return res.status(400).json({ error: 'Кейс будет доступен позже.', timeLeftMs: cooldown - timeElapsed });
        }
        ...
        if (drops.length === 0) {
            await client.query('ROLLBACK');
            return res.status(500).json({ error: 'Дроп-лист пуст. Обратитесь к админу.' });
        }
        ...
        await client.query('INSERT INTO transactions (user_id, type, item_id, amount, details) VALUES ($1, $2, $3, $4, $5)',
            [userId, 'case_open', wonItem.item_id, wonItem.value, 'Выигрыш: ' + wonItem.name]);

        await client.query('COMMIT');
        res.json({ success: true, wonItem: { id: wonItem.item_id, name: wonItem.name, price: wonItem.value + " TON" }, newBalance });
    } catch (error) {
        await client.query('ROLLBACK');
        console.error(error);
        res.status(500).json({ error: 'Ошибка транзакции кейса.' });
    } finally {
        client.release();
    }
});

// БЕЗОПАСНАЯ ПРОДАЖА: Цена берется исключительно из базы данных!
app.post('/api/sell_gift', async (req, res) => {
    if (!req.telegramUser || !req.telegramUser.id) return res.status(401).json({ error: 'Unauthorized' });
    const userId = req.telegramUser.id;
    const { itemId } = req.body; // Больше не доверяем price с фронтенда!
    ...
    try {
        ...
        // 2. Получаем ЕГО РЕАЛЬНУЮ ЦЕНУ ИЗ ТАБЛИЦЫ ITEMS
        const itemDetailsRes = await client.query('SELECT value, name FROM items WHERE id = $1', [itemId]);
        const itemDetails = itemDetailsRes.rows[0];
        
        if (!itemDetails) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: 'Предмет не существует в системе.' });
        }

        const itemRealPrice = parseFloat(itemDetails.value);

        // 3. Списываем предмет
        await client.query('UPDATE user_inventory SET quantity = quantity - 1 WHERE user_id = $1 AND item_id = $2', [userId, itemId]);
        
        // 4. Обновляем баланс юзера
        const userRes = await client.query('SELECT balance FROM users WHERE id = $1 FOR UPDATE', [userId]);
        const newBalance = parseFloat(userRes.rows[0].balance) + itemRealPrice;

        await client.query('UPDATE users SET balance = $1 WHERE id = $2', [newBalance, userId]);
        await client.query('INSERT INTO transactions (user_id, type, item_id, amount, details) VALUES ($1, $2, $3, $4, $5)',
            [userId, 'item_sold', itemId, itemRealPrice, 'Продажа предмета: ' + itemDetails.name]);
...
app.post('/api/deposit_gift_request', async (req, res) => {
    ...
    try {
        ...
        if (adminId && bot) {
            const userMention = user.username ? '@' + user.username : (user.first_name || 'Без имени');
            const message = `📥 *Заявка на ВВОД подарка!*\n\n🎁 *Подарок:* ${itemDetails.name}\n👤 *Отправитель:* ${user.first_name || ""} (${userMention})\n🆔 *ID:* \`${userId}\``;
            
            bot.sendMessage(adminId, message, {
                parse_mode: 'Markdown',
                reply_markup: {
                    inline_keyboard: [[
                        { text: '✅ Одобрить', callback_data: `dep_app_${userId}_${parsedItemId}` },
...
if (bot && typeof bot.on === 'function') {
    bot.on('callback_query', async (callbackQuery) => {
        ...
        if (!action.startsWith('dep_app_') && !action.startsWith('dep_rej_')) return;
        ...
        try {
            ...
            if (isApprove) {
                ...
                bot.sendMessage(targetUserId, `🎉 *Подарок зачислен!*\n\nПредмет *"${itemName}"* добавлен в инвентарь!`, { parse_mode: 'Markdown' }).catch(() => {});
                bot.editMessageText(`✅ Заявка одобрена. КЕЙС: "${itemName}" начислен!`, { chat_id: msg.chat.id, message_id: msg.message_id }).catch(() => {});
            } else {
                bot.sendMessage(targetUserId, `❌ *Ввод подарка "${itemName}" отклонен администратором.*`, { parse_mode: 'Markdown' }).catch(() => {});
                bot.editMessageText(`❌ Заявка отклонена администратором.`, { chat_id: msg.chat.id, message_id: msg.message_id }).catch(() => {});
            }
        } catch (err) {
            console.error(err);
        }
