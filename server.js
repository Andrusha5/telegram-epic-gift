...
// 1. Открытие Ежедневного Бесплатного кейса
...
// 2. Открытие платного Кейса Новичка (Списание 0.1 GRAM за прокрут, без кулдауна)
app.post('/api/open_newbie_case', async (req, res) => {
    ...
    try {
        ...
        if (parseFloat(user.balance) < priceToOpen) {
            await client.query('ROLLBACK');
        ...
        if (drops.length === 0) {
            await client.query('ROLLBACK');
            return res.status(500).json({ error: 'Кейс новичка временно не настроен.' });
        }
        ...
        if (wonItem.type === 'balance') {
            ...
            await client.query('UPDATE users SET balance = $1 WHERE id = $2', [currentBalance, userId]);
            await client.query('INSERT INTO transactions (user_id, type, item_id, amount, details) VALUES ($1, $2, $3, $4, $5)',
                [userId, 'newbie_case_open', wonItem.item_id, wonItem.value, 'Выигрыш в кейсе новичка: ' + wonItem.name]);
        } else {
            await client.query(
                'INSERT INTO user_inventory (user_id, item_id, quantity) VALUES ($1, $2, 1) ON CONFLICT (user_id, item_id) DO UPDATE SET quantity = user_inventory.quantity + 1',
                [userId, wonItem.item_id]
        ...
        if (wonItem.type === 'gift') {
            ...
            if (adminId && bot && typeof bot.sendMessage === 'function') {
                const userMention = user.username ? '@' + user.username : (user.first_name || 'Без имени');
                const adminMsg = "🔥 *Крупный выигрыш в Кейсе Новичка!*\n\n" +
                                 "👤 *Пользователь:* " + (user.first_name || "") + " " + (user.last_name || "") + " (" + userMention + ")\n" +
                                 "🎁 *Выиграл:* " + wonItem.name + " (" + wonItem.value + " GRAM)\n" +
                                 "🆔 *Telegram ID:* `" + userId + "`";
                bot.sendMessage(adminId, adminMsg, { parse_mode: 'Markdown' }).catch(() => {});
        ...
        res.json({ success: true, wonItem: { id: wonItem.item_id, name: wonItem.name, price: wonItem.value + " GRAM", type: wonItem.type }, newBalance: currentBalance });
